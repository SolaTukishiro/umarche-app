                ...
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="-m-2">
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative flex justify-around">
                                    <div><input type="radio" name="type" value="{{ \App\Constants\Common::PRODUCT_LIST['add'] }}" class="mr-2" checked>追加</div>
                                    <div><input type="radio" name="type" value="{{ \App\Constants\Common::PRODUCT_LIST['reduce'] }}" class="mr-2" >削減</div>
                                </div>
                            </div>
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative">
                                        ...
                                        @foreach($categories as $category)
                                            <optgroup label="{{ $category->name }}">
                                                @foreach($category->secondary as $secondary)
                                                    <option value="{{ $secondary->id}}" @if($secondary->id === $product->secondary_category_d) selected @endif>
                                                        {{ $secondary->name }}
                                                    </option>
                                                @endforeach
                                        @endforeach
                    ...
                    <form id="delete_{{$product->id}}" method="post" action="{{route('owner.products.destroy', ['product'=>$product->id])}}">
                        ...
                        <div class="p-2 w-full mt-32 flex justify-around">
                            <a href="#" data-id="{{$product->id}}" onclick="deletePost(this)" class="flex mx-auto text-white bg-red-500 border-0 py-2 px-8 focus:outline-none hover:bg-red-600 rounded">削除する</a>
                        </div>
                    </form>
                </div>
            </div>
